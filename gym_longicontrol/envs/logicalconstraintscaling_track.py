...
class LogicalConstraintScaling(gym.Env):
    ...
    def __init__(self,
        ...
        print(self.speed_limit_positions)
        print(self.speed_limits)
        #adds -inf to speed_limit_positions and copies first speed limit (in case car goes backwards)

        #self.speed_limit_positions_end = speed_limit_positions[1:]
        #self.speed_limit_positions_end.append(self.track_length)
    
        self.current_speed_limit = 0.0
        self.current_speed_limit_position = 0.0
        

        (self.current_speed_limit, self.current_speed_limit_position, 
        self.future_speed_limits, self.future_speed_limit_distances, 
        self.previous_speed_limit, self.previous_speed_limit_position, self.next_speed_limit, self.next_speed_limit_position) = self.sensor(self.position)

        self.previous_speed_limit = self.current_speed_limit 
        #previous speed limit is same as current speed limit at the beginning 

        #self.current_speed_limit_position = self.speed_limit_positions[2]
        #self.previous_speed_limit_position = self.speed_limit_positions[2]
        #speed limit positions for previous and current are the same  -- both END at this index 
         
        #should start at this index -- i literally want to die 
        self.current_speed_limit_position = self.speed_limit_positions[1]
        self.previous_speed_limit_position = self.speed_limit_positions[1]
        #the speed limits START AT THIS POSITION
        ...
        self.monitor_curr = {
            ...
            'current_speed_limit_position' : self.current_speed_limit_position,
            'previous_speed_limit' : self.previous_speed_limit,
            'previous_speed_limit_position' : self.previous_speed_limit_position,
            'track_length' : self.track_length,
            'velocity' : self.velocity,
            'position' : self.position,
            'next_speed_limit_position': self.next_speed_limit_position,
        ...
        self.monitor_param_curr = {
            'braking' : self.car.specs['acceleration_limits'][0],
            'acc' : self.car.specs['acceleration_limits'][1]
        }

        self.monitor_param_prev = {
            'braking' : self.car.specs['acceleration_limits'][0],
            'acc' : self.car.specs['acceleration_limits'][1]
        }
    ...
    def step(self, action):
        ...
        (self.current_speed_limit, self.current_speed_limit_position, self.future_speed_limits,
         self.future_speed_limit_distances, self.previous_speed_limit, self.previous_speed_limit_position, 
         self.next_speed_limit, self.next_speed_limit_position) = self.sensor(self.position)
        ...
        #current == state we are transitioning to AFTER we commit this action 
        #previous == state we WERE at 
        self.monitor_prev = {
            'acceleration' : self.monitor_curr['acceleration'],
            'prev_acceleration': self.monitor_curr['prev_acceleration'],
            'current_speed_limit' : self.monitor_curr['current_speed_limit'],
            'current_speed_limit_position' : self.monitor_curr['current_speed_limit_position'],
            'previous_speed_limit' : self.monitor_curr['previous_speed_limit'],
            'previous_speed_limit_position' : self.monitor_curr['previous_speed_limit_position'],
            'track_length' : self.monitor_curr['track_length'],
            'velocity' : self.monitor_curr['velocity'],
            'position' : self.monitor_curr['position'],
            'next_speed_limit_position': self.monitor_curr['next_speed_limit_position'],
            'next_speed_limit': self.monitor_curr['next_speed_limit']
        }
        ...
        self.monitor_param_prev = {
            'braking' : self.monitor_param_curr['braking'],
            'acc' : self.monitor_param_curr['acc']
        }
    ...
    def reset(self):
        ...
        self.current_speed_limit_position = self.speed_limit_positions[1]
        self.previous_speed_limit_position = self.speed_limit_positions[1]
        #speed limits START AT THIS POSITION 

        #self.current_speed_limit_position = self.speed_limit_positions[2]
        #self.previous_speed_limit_position = self.speed_limit_positions[2]
        #speed limits END AT THIS POSITION

        state = self.feature_scaling(self.get_state())


        #reseting dictionaries again 
        self.monitor_curr = {
            'acceleration' : self.acceleration,
            'prev_acceleration': self.prev_acceleration,
            'current_speed_limit' : self.current_speed_limit,
    ...
    def get_reward(self):
        ...
        #scaling will occur during the calculation of the monitor value within the formal methods (inside monitor.py and scaling.py)
        #see scaling.py for relevant functions to apply 
    ...
    def sensor(self, position):
        previous_speed_limit = 0.0 
        previous_speed_limit_position = 0.0
        current_speed_limit = 0.0
        current_speed_limit_position = 0.0
        ...
        next_speed_limit_mon = 0.0 
        next_speed_limit_position_mon = 0.0 

        for i, (pos, sl) in enumerate(
                zip(self.speed_limit_positions, self.speed_limits)):
            if pos <= position: #at least one of these pos will be less than current position
                current_speed_limit = sl
                current_speed_limit_i = i
                #if i+1 >= len(self.speed_limit_positions): 
                #    current_speed_limit_position = self.track_length
                #else: current_speed_limit_position = self.speed_limit_positions[i+1]

                if i >= len(self.speed_limit_positions): 
                   current_speed_limit_position = self.track_length
                else: current_speed_limit_position = self.speed_limit_positions[i]
                #i should not be adding +1, i should be using the beginning of the speed limit 

                if current_speed_limit == self.current_speed_limit: #does not change 
                    previous_speed_limit = self.previous_speed_limit
                    previous_speed_limit_position = self.previous_speed_limit_position
                else: #updates previous and current
                    previous_speed_limit = self.current_speed_limit
                    previous_speed_limit_position = self.current_speed_limit_position
        if current_speed_limit_i + 1 > len(self.speed_limits) - 1:
            ...
            next_speed_limit_mon = current_speed_limit
            next_speed_limit_position_mon = self.track_length 
            ...
            next_speed_limit_mon = self.speed_limits[current_speed_limit_i + 1]
            next_speed_limit_position_mon = self.speed_limit_positions[
                current_speed_limit_i + 1]
        ...
        return (current_speed_limit, current_speed_limit_position, future_speed_limits,
                future_speed_limit_distances, previous_speed_limit, previous_speed_limit_position,
                next_speed_limit_mon, next_speed_limit_position_mon)
    ...
    def render(self, mode='human'):
        ...
        if math.floor(self.velocity*3.6) > math.floor(self.current_speed_limit*3.6):
